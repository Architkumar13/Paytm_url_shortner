...
2. **Postgres + interface vs. SQLite/in-memory.** The interface earns its keep by making tests
...
- **Link analytics** (click counts, last-access, referrer breakdown, a stats endpoint) — hinted at
  by the title but not in the stated requirements, so I deliberately descoped it to focus on the
  core. I'd add it behind an async, batched ingestion path (channel + worker, or an outbox) so the
  writes stay off the hot redirect path.
